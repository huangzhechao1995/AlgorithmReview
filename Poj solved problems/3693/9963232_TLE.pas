program poj3693;
 ...
 var s,s1:ansistring;
     ...
     f,dp:array[1..maxn,0..20] of longint;
...
function minner(a,b:longint):longint;
 begin
  if rk[a]<rk[b] then exit(a); exit(b);
 end;
procedure STMIN(n:longint);
 var i,j,p,k,l:longint;
 begin
// fillchar(f,sizeof(f),0);
 for i:=0 to 20 do
 ...
 for k:=1 to trunc(ln(n)/ln(2)) do
  for i:=1 to n-er[k]+1 do
   dp[i,k]:=minner(dp[i,k-1],dp[i+er[k-1],k-1]);
 end;
function small(l,r:longint):longint;
 var i,j,p:longint;
 begin
 if l>r then
  begin
 ...
 small:=0;
 p:=trunc(ln(r-l+1)/ln(2));
 small:=minner(dp[l,p],dp[r+1-er[p],p]);
 end;
procedure suffix;
 begin
...
// fillchar(h,sizeof(h),0);
 ...
 for i:=1 to nn do
  ...
  j:=sa[rk[i]-1];
  while (i+p<=Nn)and(j+p<=Nn)and(s[i+p]=s[j+p]) do inc(p);
  h[rk[i]]:=p;
  if p>0 then dec(p);
  end;
 end;
procedure main;
 ...
 // if n mod len=0 then
  ...
  while j<=n do
   ...
   k1:=ask(rk[i],rk[j]);
   kk:=k1+k2-1; if kk<0 then kk:=0;
   k:=kk div len+1;
 { if (k>maxk) then
    begin
    maxk:=k; maxj:=p; maxkk:=k*len;
    end
   else
   if k=maxk then}
   IF(K>MAXK)OR(K=MAXK) THEN
   p:=small(i-k2+1,j+k1-k*len);
   if (k>maxk)or(k=maxk)and(rk[p]<rk[maxj]) then
    begin
    maxk:=k; maxj:=p; maxkk:=k*len;
    end;
   i:=j; j:=i+len;
