...
procedure expand(x,y,k:longint);
 ...
 for i:=1 to 8 do
  ...
  if a[x,y]in ['-','%'] then if nx>x then continue;
  if (a[nx,ny]in ['-','%'])and(k=0)and(nx<x) then
   begin
...
// openfile;
 ...
 for i:=1 to n do
  ...
  if a[i][j]='X' then
   ...
   k:=m;
   while a[i][k]<>'X' do
    begin if a[i][k]='.' then a[i][k]:='-' else a[i][k]:='%'; dec(k); end;
   break;
   end;
  if k<>0 then break;
  end;
 for i:=1 to n do
  for j:=1 to m do
  if a[i][j]in ['*','%'] then begin sx:=i; sy:=j; end;
